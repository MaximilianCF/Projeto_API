...
from app.routes.v1 import (cdi, desafios, ibov, infomoney, ipca, leaderboard,
                           me, protected, selic, sp500, status, submissoes,
                           token, treasury, upload, usd_brl, users)
...
def include_v1_routes(app: FastAPI):
    ...
    app.include_router(
        infomoney.router, prefix="/api/v1/webscraping", tags=["WEBSCRAPING"]
    )
    app.include_router(protected.router, prefix="/api/v1")
    app.include_router(status.router, prefix="/api/v1", tags=["Status"])
    app.include_router(upload.router, prefix="/api/v1/upload", tags=["Upload"])
    app.include_router(
        submissoes.router, prefix="/api/v1/submissoes", tags=["Submissões"]
    )
    app.include_router(
        desafios.router,
        prefix="/api/v1/desafios",
        tags=["Desafios"])
    app.include_router(
