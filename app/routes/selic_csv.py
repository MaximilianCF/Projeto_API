...
async def get_selic_csv(
    ...
    try:
        ...
        headers = {"Accept": "text/csv", "User-Agent": "python-httpx/0.27.0"}
        ...
        for tentativa in range(3):
            try:
                ...
                logger.debug(
                    f"Resposta (primeiros 500 caracteres): {response.text[:500]}"
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao processar CSV da SELIC: {detail}"
        )
